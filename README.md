...
Switch providers from **veltez → Settings → General**. Only the fields for the selected provider are shown — no clutter. All three providers enforce a `max_tokens` cap of 1024 per request to prevent runaway cost.
...
- Admin page: **veltez → Quick Replies**
...
- CSV export
- Admin page: **veltez → Top Requests**
...
Go to **veltez → Settings → General** in your WordPress admin sidebar.
...
2. Click **Create new secret key** — give it a name (e.g. `veltez`)
...
2. Click **Create Key** — name it (e.g. `veltez`)
3. Copy the key
...
The plugin adds a top-level **veltez** menu to the WordPress admin with eight sub-pages:

### Chat History (`veltez → Chat History`)
...
### IP Blocklist (`veltez → IP Blocklist`)
...
### Quick Replies (`veltez → Quick Replies`)
...
### Top Requests (`veltez → Top Requests`)
...
### AI Error Log (`veltez → AI Error Log`)
...
### Plugin Guide (`veltez → Plugin Guide`)
