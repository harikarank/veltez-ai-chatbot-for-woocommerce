...
final class Plugin {
	...
	public function handle_admin_post_qr_save() {
		$this->get_quick_reply_service()->handle_save();
	}

	public function handle_admin_post_qr_delete() {
		$this->get_quick_reply_service()->handle_delete();
	}

	public function handle_admin_post_qr_save_from_ai() {
		$this->get_quick_reply_service()->handle_save_from_ai();
	}

	public function handle_admin_post_ip_add() {
		$this->ip_blocker->handle_add();
	}

	public function handle_admin_post_ip_delete() {
		$this->ip_blocker->handle_delete();
	}
	...
	public function add_admin_bar_node( \WP_Admin_Bar $wp_admin_bar ) {
		...
		foreach ( $subitems as $item ) {
			$wp_admin_bar->add_node( array_merge( $item, array( 'parent' => 'veltez-ai-bar' ) ) );
		}
	}
