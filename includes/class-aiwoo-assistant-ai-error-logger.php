		...
		try {
			...
			$result = $wpdb->insert(
				...
				array(
					...
					'error_message' => $strip4( sanitize_textarea_field( (string) $error_message ) ),
			...
			if ( false === $result && '' !== $wpdb->last_error ) {
				...
				error_log( '[veltez] Error log insert failed: ' . $wpdb->last_error );
			}
		} catch ( \Throwable $e ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( '[veltez] Error log exception: ' . $e->getMessage() );
		}
	}
