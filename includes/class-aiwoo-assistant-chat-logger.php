...
final class Chat_Logger {
  ...
  KEY idx_created_at (created_at)
	...
	public function log( $session_id, $ip_address, $user_message, $ai_response ) {
		...
		// Strip 4-byte Unicode characters (emoji, supplementary CJK, etc.) so the
		// INSERT never fails silently on sites whose DB uses the 3-byte utf8 charset.
		$strip4 = static function ( $str ) {
		...
		try {
			...
			$result = $wpdb->insert(
				...
				array(
					...
					'customer_name' => '',
					'user_message'  => $strip4( sanitize_textarea_field( (string) $user_message ) ),
					'ai_response'   => $strip4( wp_strip_all_tags( (string) $ai_response ) ),
			...
			if ( false === $result && '' !== $wpdb->last_error ) {
				...
				error_log( '[veltez] Chat log insert failed: ' . $wpdb->last_error );
			}
		} catch ( \Throwable $e ) {
			// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			error_log( '[veltez] Chat log exception: ' . $e->getMessage() );
		}
	}
	...
	public function get_sessions( array $filters = array(), $per_page = 20, $offset = 0 ) {
		...
		[ $where_tpl, $where_args ] = $this->build_where( $filters );

		// phpcs:disable WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber, WordPress.DB.DirectDatabaseQuery, PluginCheck.Security.DirectDB.UnescapedDBParameter
		if ( empty( $where_args ) ) {
	...
	/**
	 * Build a WHERE template and corresponding args for use with $wpdb->prepare().
	 *
	 * Returns [ string $sql_template, array $args ] where $sql_template contains
	 * %s / %d placeholders and $args holds the corresponding values, ready to be
	 * spread into a $wpdb->prepare() call.
	 *
	 * @param array $filters  Keys: ip, name, date_from, date_to (all optional).
	...
	private function build_where( array $filters ) {
		...
		if ( ! empty( $filters['ip'] ) ) {
			...
			$args[]       = sanitize_text_field( $filters['ip'] );
		}

		if ( ! empty( $filters['name'] ) ) {
			$conditions[] = 'customer_name LIKE %s';
			$args[]       = '%' . $wpdb->esc_like( sanitize_text_field( $filters['name'] ) ) . '%';
		}
		...
		return array( implode( ' AND ', $conditions ), $args );
	}
}
